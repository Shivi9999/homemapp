...
def customer_dashboard(request):
    return render(request,'user/dash_user.html')
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if user.user_type == '3':
                ...
                request.session['customer_session_id'] = user.id
                customer_session_id = request.session.get('customer_session_id')
                print(f"Customer Session ID: {customer_session_id}")

                return redirect('customer_dashboard')  # User dashboard
...
def logout_view_customer(request):
   # Clear the session
    ...
    response = redirect('login_customer')
...
def get_answer_cutomer(request):
    error_message = ''
    user_input = ''
    ...
    try:
        if request.method == 'GET':
            ...
            if not user_input:
                ...
                # Assuming you have information about the current user
                current_user = request.user

                # Fetch the user's room number
                user_room_number = get_object_or_404(Add_user, user=current_user).room_number

                # Now, fetch the answer based on the user's room number
